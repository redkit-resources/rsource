...
void CSkeletalAnimation::LoadFromLipsyncCache( IFile* inFile )
{
	if( inFile == nullptr || m_animBuffer != nullptr )
		return;

	m_animBuffer = ClassID< CAnimationBufferBitwiseCompressed >( )->CreateObject< CAnimationBufferBitwiseCompressed >( );
	if( m_animBuffer != nullptr )
	{
		CAnimationBufferBitwiseCompressed* buf = reinterpret_cast< CAnimationBufferBitwiseCompressed* >( m_animBuffer );

		*inFile << m_framesPerSecond;
		*inFile << m_duration;

		buf->m_bones.SerializeBulk( *inFile );
		buf->m_tracks.SerializeBulk( *inFile );
		buf->m_data.SerializeBulk( *inFile );
		*inFile << buf->m_numFrames;
		*inFile << buf->m_dt;

		buf->m_duration = m_duration;
	}
}

void CSkeletalAnimation::SaveToLipsyncCache( IFile* outFile )
{
	if( outFile == nullptr )
		return;

	if( CAnimationBufferBitwiseCompressed* buf = reinterpret_cast< CAnimationBufferBitwiseCompressed* >( m_animBuffer ) )
	{
		*outFile << m_framesPerSecond;
		*outFile << m_duration;

		// Ready for the fakefest? Thanks ANIM_COMPRESSION_SCALE_OPTS! ...NO.
		{
			struct SFakeAnimationBufferBitwiseCompressedBoneTrack
			{
				SAnimationBufferBitwiseCompressedData m_position;
				SAnimationBufferBitwiseCompressedData m_orientation;
			};
			TDynArray< SFakeAnimationBufferBitwiseCompressedBoneTrack > fakeBones( buf->m_bones.Size( ) );
			for( Uint32 t = 0; t < fakeBones.Size(); ++t )
			{
				fakeBones[ t ].m_position = buf->m_bones[ t ].m_position;
				fakeBones[ t ].m_orientation = buf->m_bones[ t ].m_orientation;
			}
			fakeBones.SerializeBulk( *outFile );
		}
		buf->m_tracks.SerializeBulk( *outFile );
		buf->m_data.SerializeBulk( *outFile );
		*outFile << buf->m_numFrames;
		*outFile << buf->m_dt;
	}
}