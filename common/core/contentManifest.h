...
struct SContentPackInstance
{
	...
	void SetIsStreamingDLC( Bool isStreamingDLC )
	{
		m_isStreamingDLC = isStreamingDLC;
	}
...
enum EContentPackageMountFlags
{
	...
	eContentPackageMountFlags_AutoAttachChunks	= FLAG(0), //<! Bit hacky, for DLC attach the chunks instead of waiting for the streaming installer
	eContentPackageMountFlags_IsHidden			= FLAG(1), //!< Bit hacky, for DLC don't report as installed content - hidden. Query the DLC manager instead.
	eContentPackageMountFlags_IsStreamingDLC	= FLAG(2), //!< Bit hacky yet again, for DLC that's on the disc and can probably be "hot loaded" at certain points instead of the main menu only.
};
