	...
	for ( Int32 i = 0; i < m_attachedPackageIDs.SizeInt(); ++i )
	{
		...
		Bool isStreamingDLC = false;
		{
			...
			if ( findIt != m_packageInstanceMap.End() )
			{
				...
				isStreamingDLC = packInstance->m_isStreamingDLC;
			}
			else
			{
				ERR_CORE(TXT("Can't find pack instance for packageID %u"), packageID );
			}
		}
		...
		for ( SContentListenerContext& listenerContext : m_contentListeners )
		{
			if ( listenerContext.m_maxNotifiedPackageIndex < i )
			{
				...
				listenerContext.m_contentListener->OnPackageAvailable( packageID, isStreamingDLC );
			}
		}
	}
}
...
void CContentManager::OnPackageMounted( RuntimePackageID packageID, const String& mountPath, Uint32 mountFlags )
{
	...
	const Bool isStreamingDLC = ( mountFlags & eContentPackageMountFlags_IsStreamingDLC ) != 0;

	LOG_CORE(TXT("CContentManager[OnPackageMounted] packageID=%u, mountPath='%ls', isAutoAttachChunks=%d, isHidden=%d isStreamingDLC=%d"), 
		packageID, mountPath.AsChar(), (isAutoAttachChunks ? 1 : 0), (isHidden? 1 : 0 ), (isStreamingDLC ? 1 : 0));
	SContentPackInstance* packInstance = nullptr;
	auto findIt = m_packageInstanceMap.Find( packageID );
	if ( findIt == m_packageInstanceMap.End() )
	{
		...
		packInstance->SetIsStreamingDLC( isStreamingDLC );
