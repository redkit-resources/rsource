...
void CFactsDB::OnLoadGame( IGameLoader* loader )
{
	...
	// Mega censorship hack. This release_jp was already used to determine things like should we play a censored version of a cutscene
	// Now we need to add it after game start if using an uncensored savegame but with "Arabic censorship".
	// However, we need to remove it if you then play WITHOUT "Arabic censorship" but NOT remove it if it was added for
	// some other reason. Hence the "release_jpTransient" fact to say that it was added because of Arabic.
	// Yeah. And _jp also applies to Arabic, just Japanese censorship came first and this was the fact name chosen.
	static const Bool isArabic = (Red::StringSearch( SGetCommandLine(), TXT("-arabic") ) != nullptr);
	const String releaseJP = TXT( "release_jp" );
	const String releaseJPTransient = TXT( "release_jpTransient" );
	if ( isArabic )
	{
		// Only add "transient" fact if not already "releaseJP"
		if ( !DoesExist( releaseJP ) )
		{
			AddFact( releaseJP, 1, GGame->GetEngineTime(), CFactsDB::EXP_NEVER_SEC );
			AddFact( releaseJPTransient, 1, GGame->GetEngineTime(), CFactsDB::EXP_NEVER_SEC );
		}
	}
	else
	{
		if ( DoesExist( releaseJPTransient ) )
		{
			RemoveFact( releaseJP );
			RemoveFact( releaseJPTransient );
		}
	}
}
