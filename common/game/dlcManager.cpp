...
void CDLCManager::OnPackageAvailable( RuntimePackageID packageID, Bool isStreamingDLC )
{
	if ( packageID != BASE_RUNTIME_PACKAGE_ID )
	{
		...
		if( m_displayNewDLCInstalled && GGame->IsActive() && !isStreamingDLC )
		{
#ifdef PLATFORM_PS4
			...
			m_displayNewDLCInstalled = false;
		}	
		m_isNewContentAvailable = true;
	}
}

void CDLCManager::Scan(EScanType scanType /*= EScanType::Normal */)
{
	...
	static Bool isGOTY = Red::System::StringSearch( SGetCommandLine(), TXT( "-goty" ) ) != nullptr;
#else
	// Just preinstall the DLCs
	static Bool isGOTY = false;
#endif

	if ( scanType == EScanType::Hotload && !isGOTY )
		return;

	// Don't reset because we might to scan again during normal loading for non-hotloadable DLC
	if ( scanType == EScanType::Normal )
	{
		m_isNewContentAvailable = false;
		m_displayNewDLCInstalled = true;
	}
	...
	// We'll get a package event when the DLC is really installed, so we'll scan again
#ifdef RED_PLATFORM_CONSOLE
	...
	// #hack: list of DLCs that are stream installed in the GOTY
	// which we should activate when we see them
	static const Char* const GOTY_DLCs[] = { 
		TXT("bob.reddlc"), TXT("ep1.reddlc"), 
		TXT("dlc1.reddlc"), TXT("dlc2.reddlc"), TXT("dlc3.reddlc"), TXT("dlc4.reddlc"),
		TXT("dlc5.reddlc"), TXT("dlc6.reddlc"), TXT("dlc7.reddlc"), TXT("dlc8.reddlc"),
		TXT("dlc9.reddlc"), TXT("dlc10.reddlc"), TXT("dlc11.reddlc"), TXT("dlc12.reddlc"),
		TXT("dlc13.reddlc"), TXT("dlc14.reddlc"), TXT("dlc15.reddlc"), TXT("dlc16.reddlc")
	};
	...
	for ( CDirectory* dlcDir : dir->GetDirectories() )
	{
		for ( CDiskFile* dlcFile : dlcDir->GetFiles() )
		{
			...
			{
				Bool addDlcFile = false;
				if ( scanType == EScanType::Hotload )
				{
					if ( isGOTY )
					{
						const String& fileName = dlcFile->GetFileName();
						for ( Uint32 i = 0; i < ARRAY_COUNT_U32(GOTY_DLCs); ++i )
						{
							const Char* const dlcName = GOTY_DLCs[i];
							if ( fileName.EqualsNC(dlcName) )
							{
								addDlcFile = true;
								break;
							}
				...
				{
					addDlcFile = dlcFile->IsRuntimeAttached() || !dlcRuntimeInstall;
				}
				
				LOG_GAME(TXT("dlcFile '%ls', isRuntimeAttached=%d; addDlcFile=%d, dlcRuntimeInstall=%d scanType=%ls"),
					dlcFile->GetDepotPath().AsChar(), 
					dlcFile->IsRuntimeAttached(),
					addDlcFile,
					dlcRuntimeInstall,
					(scanType == EScanType::Hotload? TXT("hotload") : TXT("normal")));
