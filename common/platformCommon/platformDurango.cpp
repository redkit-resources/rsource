...
void CPlatform::Initialize( const String& commandLine, const Core::CommandLineArguments& coreArguments )
{
	...
	if ( streamInstall )
	{
		...
		Durango::GGOTYOnDiscDLCInstaller = new CStreamingDLCInstaller;
		if ( !Durango::GGOTYOnDiscDLCInstaller->Init() )
		{
			delete Durango::GGOTYOnDiscDLCInstaller;
			Durango::GGOTYOnDiscDLCInstaller = nullptr;
			RED_FATAL( "Could not create CGOTYOnDiscDLCInstaller" );
		}

		GContentManager->RegisterContentInstaller( Durango::GGOTYOnDiscDLCInstaller );
		GContentManager->RegisterContentInstaller( Durango::GStreamingInstaller );
		GContentManager->RegisterContentListener( Durango::GGOTYOnDiscDLCInstaller, "*.reddlclink" );
	}
	else
	{
