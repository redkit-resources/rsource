...
#include "../core/streamingDLCInstaller.h"
...
namespace Orbis
{
	...
	CStreamingDLCInstaller* GGOTYOnDiscDLCInstaller;
...
void CPlatform::Initialize( const String& commandLine, const Core::CommandLineArguments& coreArguments )
{
	...
	if ( streamInstall )
	{
		...
		// Uses the streaming installer, so if -streaminstall, not if -dlcinstall (which is the runtime DLC installer)
		Orbis::GGOTYOnDiscDLCInstaller = new CStreamingDLCInstaller;
		if ( !Orbis::GGOTYOnDiscDLCInstaller->Init() )
		{
			delete Orbis::GGOTYOnDiscDLCInstaller;
			Orbis::GGOTYOnDiscDLCInstaller = nullptr;
			RED_FATAL( "Could not create CGOTYOnDiscDLCInstaller" );
		}

		GContentManager->RegisterContentInstaller( Orbis::GGOTYOnDiscDLCInstaller );
		GContentManager->RegisterContentInstaller( Orbis::GStreamingInstaller );
		GContentManager->RegisterContentListener( Orbis::GGOTYOnDiscDLCInstaller, "*.reddlclink" );
	}
	else
	{
...
void CPlatform::Shutdown()
{
	...
	GContentManager->UnregisterContentListener( Orbis::GGOTYOnDiscDLCInstaller );
