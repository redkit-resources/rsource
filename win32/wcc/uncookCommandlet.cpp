...
private:
	...
	public:
		...
		~AutoDiscardedPtr( ) { if( ptr ) ptr->Discard( ); }
...
Bool CUncookCommandlet::ProcessBundleFile( const String& absoluteFilePath )
{
	...
	for( Uint32 itemIndex = 0; itemIndex < headerItemCount; ++itemIndex )
	{
		...
		if( existingSize == fileHeaderInfo.m_dataSize )
			continue;
#ifdef WCC_LITE
		if (fullPath.ContainsSubstring(TXT("pub\\xbox1")) || fullPath.ContainsSubstring(TXT("qa")) || fullPath.ContainsSubstring(TXT("test_cases")) )
		{
			continue;
		}
#endif
