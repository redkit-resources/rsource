...
CPackageProjectBuilderPS4::CPackageProjectBuilderPS4( IFile& batchFileWriter, const SPackageProjectOptionsPS4& packageOptions, const SPackageSkuPS4& packageSku )
...
Bool CPackageProjectBuilderPS4::BuildPatchProject( TDynArray< SPackageContentManifestPS4 >& outPackageContentManifests )
{
	...
	const Uint32 gotyLayer = 0; //we can remove it
	Uint32 gotyStreamingCacheChunkID = INVALID_CHUNK_ID;
	Uint32 gotyEP1ChunkID = INVALID_CHUNK_ID;
	Uint32 gotyEP2ChunkID = INVALID_CHUNK_ID;
	...
	for ( const auto& it : basePackage.m_chunks )
	{
		...
		else if ( it.m_label.EndsWith(TXT("_ep1")) )
		{
			gotyEP1ChunkID = it.m_chunkID;
			if ( it.m_layer != gotyLayer )
			{
				ERR_WCC(TXT("Unexpected EP1 layer %u"), it.m_layer);
				return false;
			}
		}
		else if ( it.m_label.EndsWith(TXT("_ep2")) )
		{
			gotyEP2ChunkID = it.m_chunkID;
			if ( it.m_layer != gotyLayer )
			{
				ERR_WCC(TXT("Unexpected EP2 layer %u"), it.m_layer);
				return false;
			}
		}
		else if ( it.m_label.EndsWith(TXT("_streamingcache")) )
		{
			if ( gotyStreamingCacheChunkID != INVALID_CHUNK_ID )
			{
				ERR_WCC(TXT("Patching multiple streaming caches not supported at the moment! More than just for Novigrad?"));
				return false;
			}
			gotyStreamingCacheChunkID = it.m_chunkID;
			if ( it.m_layer != gotyLayer )
			{
				ERR_WCC(TXT("Unexpected streaming cache layer %u"), it.m_layer);
				return false;
			}
		}
	...
	for ( const auto& it : packageFiles.m_gameContentFilesMap )
	{
		...
		// #tbd: this allows replacing and adding new on-disc DLC files that can are small enough to fit. E.g., DLC1-16
		if ( !BatchAddFiles( gameFiles.m_onDiscDLCFiles, chunkLayerPair, eFileCompression_Disabled ) )
		{
			return false;
		}
		//	NO: Don't add DLC files to content manifest. We already created a DLC content.xml
		for ( const String& file : gameFiles.m_onDiscDLCFiles )
		{
			if ( file.EndsWith( RED_DLCLINK_EXT ) )
			{
				if ( !ManifestAddFile( manifestContentName, file, chunkLayerPair ) )
				{
					return false;
				}
			}
		...
		if ( gotyStreamingCacheChunkID != INVALID_CHUNK_ID )
		{
			SChunkLayerInfoPS4 streamingCacheChunkLayerPair( gotyStreamingCacheChunkID, gotyLayer );
			for ( const String& streamingCacheFile : gameFiles.m_streamingCacheFiles )
			{
				if ( ! BatchAddFiles( mergedFiles, streamingCacheChunkLayerPair, eFileCompression_Disabled ) )
				{
					return false;
				}
				//#tbd: only supporting replacing existing file at the moment, so not added to the content manifest!
				// 	if ( ! ManifestAddFiles( manifestContentName, mergedFiles, ... ) )
				// 	{
				// 		return false;
				// 	}
			}
		}
		else if ( !gameFiles.m_streamingCacheFiles.Empty() )
		{
			ERR_WCC(TXT("Couldn't find chunkID for to streaming cache GOTY files!"));
			return false;
		}
	}

	if ( gotyEP1ChunkID != INVALID_CHUNK_ID )
	{
		SChunkLayerInfoPS4 chunkLayerPair( gotyEP1ChunkID, gotyLayer );
		for ( const String& file : packageFiles.m_ep1Files )
		{
			// Don't bother adding empty link file. Adding new DLCs in a patch not supported right now anyway.
			if ( file.EndsWith( RED_DLCLINK_EXT ) )
				continue;

			if ( ! BatchAddFile( file, chunkLayerPair, eFileCompression_Disabled ) )
			{
				return false;
			}
		}
	}
	else if ( !packageFiles.m_ep1Files.Empty() )
	{
		ERR_WCC(TXT("Couldn't find chunkID for to patch EP1 GOTY files!"));
		return false;
	}

	if ( gotyEP2ChunkID != INVALID_CHUNK_ID )
	{
		SChunkLayerInfoPS4 chunkLayerPair( gotyEP2ChunkID, gotyLayer );
		for ( const String& file : packageFiles.m_ep2Files )
		{
			// Don't bother adding empty link file. Adding new DLCs in a patch not supported right now anyway.
			if ( file.EndsWith( RED_DLCLINK_EXT ) )
				continue;
	...
	else if ( !packageFiles.m_ep2Files.Empty() )
	{
		ERR_WCC(TXT("Couldn't find chunkID for to patch EP2 GOTY files!"));
		return false;
	}
...
Bool CPackageProjectBuilderPS4::BatchAddLanguageMaskChunks( Uint32 skipContent, Uint32 maxContentCount, Bool chunkAscending, Uint32 startChunkID, Uint32 layer, TLanguageMap& outLangMap, Uint32& outNextFreeChunkID )
{
	...
	for ( auto it = packageFiles.m_gameContentFilesMap.Begin(); it != packageFiles.m_gameContentFilesMap.End(); ++it )
	{
		...
		// #hack: add a new "language" chunk for the streaming cache. Need to put that file on layer1, but not all of content4 or else can't fit
		if ( sortedContentNumber == HACK_NOVIGRAD_CONTENT_NUMBER )
		{
			const Char* const fakeNames[] = { HACK_STREAMING_CACHE_LANGNAME, HACK_EP1_LANGNAME, HACK_EP2_LANGNAME };
			for ( Uint32 i = 0; i < ARRAY_COUNT_U32(fakeNames); ++i )
			{
				const Char* fakeName = fakeNames[i];
				SGameContentLanguage gameContentLanguage( fakeName, sortedContentNumber );
				SChunkLayerInfoPS4 langChunkLayerPair;
				if ( ! outLangMap.Find( gameContentLanguage, langChunkLayerPair ) )
				{
					...
					const String& label = String::Printf(TXT("content%u_%ls"), sortedContentNumber, fakeName );
					if ( ! m_batchCommandWriter.ChunkAdd( langChunkLayerPair, label ) )
					{
						return false;
					}
				}
...
Bool CPackageProjectBuilderPS4::BatchAddGameContentChunksNoLanguageMask( Uint32 skipContent, Uint32 maxContentCount, Uint32 startChunkID, Uint32 layer, TGameContentMap& outGameContentMap, Uint32& outNextFreeChunkID )
{
	...
	for ( auto it = packageFiles.m_gameContentFilesMap.Begin(); it != packageFiles.m_gameContentFilesMap.End(); ++it )
	{
		...
		// #HACK for layer size: novigrad, EP1, EP2, put on second layer
		if ( m_packageSku.m_isGOTY && sortedContentNumber >= 10 ) // need more space on layer0
		{
			chunkLayerPair = SChunkLayerInfoPS4( chunkID, 1 );
		}
...
Bool CPackageProjectBuilderPS4::AddGameLanguageMaskFiles( const TLanguageMap& langMap )
{
	...
	if ( m_packageSku.m_isGOTY )
	{
		struct EpisodeData
		{
			const Char* m_fakeLangName;
			const TDynArray<String>* m_files;
		};

		const EpisodeData epData[] = { { HACK_EP1_LANGNAME, &packageFiles.m_ep1Files }, { HACK_EP2_LANGNAME, &packageFiles.m_ep2Files } };

		for ( Uint32 i = 0; i < ARRAY_COUNT_U32(epData); ++i )
		{
			const Char* const fakeLangName = epData[i].m_fakeLangName;
			const TDynArray<String>& epFiles = *epData[i].m_files;
			if ( !epFiles.Empty() )
			{
				const CName manifestContentName(String::Printf(TXT("%ls%u_%ls"), GetContentPrefix().AsChar(), HACK_NOVIGRAD_CONTENT_NUMBER, fakeLangName));
				const CName manifestBaseContentName(String::Printf(TXT("%ls%u"), GetContentPrefix().AsChar(), HACK_NOVIGRAD_CONTENT_NUMBER ) );

				SGameContentLanguage gameContentLanguage( fakeLangName, HACK_NOVIGRAD_CONTENT_NUMBER );
				SChunkLayerInfoPS4 langChunkLayerPair;
				if ( !langMap.Find( gameContentLanguage, langChunkLayerPair) )
				{
					ERR_WCC(TXT("Failed to find chunkID for %ls files content%u"), fakeLangName, HACK_NOVIGRAD_CONTENT_NUMBER);
					return false;
				}

				for ( const String& file : epFiles )
				{
					if ( ! BatchAddFile( file, langChunkLayerPair, eFileCompression_Disabled) )
					{
						return false;
					}
					// do not add files to manifest, DLC files so have their own!
					// still need to create an empty manifest so the content/chunks will be added to the install.dat file, even though

					if ( file.EndsWith( RED_DLCLINK_EXT ) )
					{
						if ( ! ManifestAddFile( manifestContentName, manifestBaseContentName, file, langChunkLayerPair, fakeLangName ) )
						{
							return false;
						}
					}
	...
	for ( auto it = packageFiles.m_gameContentFilesMap.Begin(); it != packageFiles.m_gameContentFilesMap.End(); ++it )
	{
		...
		if ( m_packageSku.m_isGOTY )
		{
			if ( !gameFiles.m_streamingCacheFiles.Empty() )
			{
				const CName manifestContentName(String::Printf(TXT("%ls%u_%ls"), GetContentPrefix().AsChar(), sortedContentNumber, HACK_STREAMING_CACHE_LANGNAME));
				const CName manifestBaseContentName(String::Printf(TXT("%ls%u"), GetContentPrefix().AsChar(), sortedContentNumber ) );

				SGameContentLanguage gameContentLanguage( HACK_STREAMING_CACHE_LANGNAME, sortedContentNumber );
				SChunkLayerInfoPS4 langChunkLayerPair;
				if ( !langMap.Find( gameContentLanguage, langChunkLayerPair) )
				{
					ERR_WCC(TXT("Failed to find chunkID for streaming cache files content%u"), sortedContentNumber);
					return false;
				}

				for ( const String& streamingCacheFile : gameFiles.m_streamingCacheFiles)
				{
					if ( ! BatchAddFile( streamingCacheFile, langChunkLayerPair, eFileCompression_Enabled ) )
					{
						return false;
					}
					if ( ! ManifestAddFile( manifestContentName, manifestBaseContentName, streamingCacheFile, langChunkLayerPair, HACK_STREAMING_CACHE_LANGNAME ) )
					{
						return false;
					}
				}
		...
		{
			ERR_WCC(TXT("Non-GOTY SKU has segregated streaming cache files!"));
		}
	}
...
Bool CPackageProjectBuilderPS4::AddGameContentFilesNoLanguageMask( const TGameContentMap& gameContentMap )
{
	...
	for ( auto it = packageFiles.m_gameContentFilesMap.Begin(); it != packageFiles.m_gameContentFilesMap.End(); ++it )
	{
		...
		// #tbd: compression
		// #tbd: should try to language mask, but for GOTY we shouldn't have many on-disc speeches
		if ( m_packageSku.m_isGOTY )
		{
			if ( !BatchAddFiles( gameFiles.m_onDiscDLCFiles, chunkLayerPair, eFileCompression_Disabled ) )
			{
		...
		{
			if ( !gameFiles.m_onDiscDLCFiles.Empty() )
			{
				ERR_WCC(TXT("On disc DLC files found, but not GOTY SKU!"));
				return false;
			}
		}
...
Bool CPackageProjectBuilderPS4::ManifestAddFiles( CName manifestContentName, CName manifestBaseContentName, const TDynArray< String >& files, const SChunkLayerInfoPS4& chunkLayerPair, const String& language )
{
	for ( const String& file : files )
	{
		...
		SContentManifestBuildInfo& info = m_contentManifestMap.GetRef( manifestContentName, SContentManifestBuildInfo( manifestContentName, manifestBaseContentName, chunkLayerPair, language ) );
