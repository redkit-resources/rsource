...
// 	}
	...
	CPackageFileCollectorXboxOne packageFileCollector( packageType, options.m_isGOTY );
	...
	// NOTE: Need to generate even for patch, so Xbox doesn't think we removed files
	if ( options.m_isGOTY && packageType != ePackageType_Dlc )
	{
	...
	SPackageSkuXboxOne packageSku( packageType, packageFiles, packageLanguages, options.m_isGOTY );
	...
	CPackageMapFileBuilderXboxOne packageMapFileBuilder( *xmlFile, packageMapFileOptions, packageSku );
