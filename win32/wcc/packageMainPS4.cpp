...
Bool PackageMainPS4( const SOptionsPS4& options )
{
	...
	CPackageFileCollectorPS4 packageFileCollector( packageType, options.m_isGOTY );
	...
	// Only if app, since not supporting adding/removing DLC files, just replacing
	// Since otherwise need to support DLC manifest merging etc
	if ( options.m_isGOTY && packageType == ePackageType_App )
	{
		if ( !packageFileCollector.GenerateOnDiscDLCContentManifest( appRoot, packageFiles ) )
		{
			return false;
		}
	}
	...
	SPackageSkuPS4 packageSku( packageType, packageFiles, packageLanguages, options.m_isGOTY );
	...
	CPackageProjectBuilderPS4 packageProjectBuilder( *batchFileWriter, packageProjectOptions, packageSku );
...
static Bool FilterLanguages( SPackageFiles& packageFiles, const SPackageLanguagesPS4& packageLanguages, const SOptionsPS4& options )
{
	...
	TDynArray< TDynArray<String>* > pFiles;
	pFiles.Reserve( 2 + packageFiles.m_gameContentFilesMap.Size() );
	pFiles.PushBack( &packageFiles.m_ep1Files );
	pFiles.PushBack( &packageFiles.m_ep2Files );
	for ( auto& it : packageFiles.m_gameContentFilesMap )
	{
		SPackageGameFiles& gameFiles = it.m_second;
		pFiles.PushBack( &gameFiles.m_speechFiles );
		pFiles.PushBack( &gameFiles.m_stringsFiles );
		pFiles.PushBack( &gameFiles.m_onDiscDLCFiles );
	}

	for ( auto& it : pFiles )
	{
		TDynArray<String>& files = *it;
		for ( Int32 j = files.SizeInt()-1; j >= 0; --j )
		{
			const String& langFile = files[j];

			if ( !langFile.EndsWith(TXT(".w3strings")) && !langFile.EndsWith(TXT(".w3speech")) )
				continue;

			if ( !langSupport.IsSupportedLanguageFile( langFile ) )
			{
				LOG_WCC(TXT("Excluding unsupported lang file '%ls'"), langFile.AsChar() );
				files.RemoveAt( j );
			}
		}
	}
...
static Bool PlayGoPackage_InitContent( SPlayGoPackage& playGoPackage, TDynArray< SPlayGoContent >& playGoContent, const TDynArray< SPackageContentManifestPS4 >& manifests, const SPackageLanguagesPS4& packageLanguages )
{
	for ( const SPackageContentManifestPS4& manifest : manifests )
	{
		...
		if ( manifestContentName != manifestBaseContentName )
		{
			...
			const Bool fakeLang = manifest.m_language == TXT("prefetch") || manifest.m_language == HACK_STREAMING_CACHE_LANGNAME || manifest.m_language == HACK_EP1_LANGNAME ||
				manifest.m_language == HACK_EP2_LANGNAME;
			if ( !fakeLang && !packageLanguages.m_gameToPlayGoLangMap.Find( manifest.m_language, playGoLang ) )
			{
...
static Bool LoadBasePackage( PackageHelpers::SPackageInfo& basePackage, const SOptionsPS4 &options )
{
	...
	const String gp4Path = filePath.GetPathString(true) + filePath.GetFileName().LeftString(36) + TXT(".gp4");
