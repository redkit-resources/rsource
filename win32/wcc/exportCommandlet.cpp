...
#ifdef WCC_LITE
	String fbxTxt;
	if ( options.GetSingleOptionValue( TXT("fbx"), fbxTxt ) )
	{
		exportOptions.m_saveFileFormat = CFileFormat( outputFileExt, fbxTxt );
	}
	else
	{
		exportOptions.m_saveFileFormat = CFileFormat( outputFileExt, TXT("2013") );
	}
#else
...
void CExportCommandlet::PrintHelp() const
{
	...
	LOG_WCC( TXT( "	 -fbx=fbxversion	   - (Optional) Specify output FBX version - 2016, 2013, 2011, 2010, 2009"));
	LOG_WCC( TXT( "" ) );
