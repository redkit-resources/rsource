...
static Bool HasUnmaskedContent( const SPackageGameFiles& gameFiles )
{
	const Bool retval =
		...
		!gameFiles.m_onDiscDLCFiles.Empty() ||
		!gameFiles.m_streamingCacheFiles.Empty();

	return retval;
}

CPackageMapFileBuilderXboxOne::CPackageMapFileBuilderXboxOne( IFile& xmlFile, const SPackageMapFileOptionsXboxOne& packageOptions, const SPackageSkuXboxOne& packageSku )
	: m_xmlWriter( xmlFile )
...
Bool CPackageMapFileBuilderXboxOne::AddGameContentFilesNoLanguages( const TGameContentMap& gameContentMap, Bool forceSpeeches /*= false*/ )
{
	...
	// #tbd: just throw EP1/EP2 onto the same chunk as novigrad instead of a separate chunk
	// might need to throttle mounting on the Xbox to prevent any stuttering?
	if ( m_packageSku.m_isGOTY )
	{
		ChunkID novigradChunkID = INVALID_CHUNK_ID;
		if ( ! gameContentMap.Find( HACK_NOVIGRAD_CONTENT_NUMBER, novigradChunkID ) || novigradChunkID == INVALID_CHUNK_ID )
		{
			ERR_WCC(TXT("Failed to find chunkID for novigrad content%u"), HACK_NOVIGRAD_CONTENT_NUMBER );
			return false;
		}
		
		const CName manifestContentName(String::Printf(TXT("%ls%u"), GetContentPrefix().AsChar(), HACK_NOVIGRAD_CONTENT_NUMBER ));

		if ( ! AddFiles( packageFiles.m_ep1Files, novigradChunkID ) )
		{
			return false;
		}
		for ( const String& file : packageFiles.m_ep1Files )
		{
			...
			{
				if ( ! ManifestAddFile( manifestContentName, file, novigradChunkID ) )
				{
					return false;
				}
			}
		...
		if ( ! AddFiles( packageFiles.m_ep2Files, novigradChunkID ) )
		{
			return false;
		}
	...
	{
		if ( !packageFiles.m_ep1Files.Empty() && !packageFiles.m_ep2Files.Empty() )
		{
			ERR_WCC(TXT("On disc EP1 and EP2 files found, but not GOTY SKU!"));
			return false;
		}
	}
	...
	for ( auto it = packageFiles.m_gameContentFilesMap.Begin(); it != packageFiles.m_gameContentFilesMap.End(); ++it )
	{
		...
		if ( m_packageSku.m_isGOTY )
		{
			if ( ! AddFiles( gameFiles.m_streamingCacheFiles, chunkID ) )
			{
				return false;
			}
			if ( ! ManifestAddFiles( manifestContentName, gameFiles.m_streamingCacheFiles, chunkID ) )
			{
				return false;
			}

			if ( ! AddFiles( gameFiles.m_onDiscDLCFiles, chunkID ) )
			{
				return false;
			}
			...
			for ( const String& file : gameFiles.m_onDiscDLCFiles )
			{
				if ( file.EndsWith( RED_DLCLINK_EXT ) )
				{
					if ( !ManifestAddFile( manifestContentName, file, chunkID ) )
					{
						return false;
					}
				}
		...
		else
		{
			...
			{
				ERR_WCC(TXT("Streaming cache file segrated but not GOTY SKU!"));
				return false;
			}
...
Bool CPackageMapFileBuilderXboxOne::ManifestAddFiles( CName manifestContentName, CName manifestBaseContentName, const TDynArray< String >& files, ChunkID chunkID, const String& language )
{
	for ( const String& file : files )
	{
		...
		SContentManifestBuildInfo& info = m_contentManifestMap.GetRef( manifestContentName, SContentManifestBuildInfo( manifestContentName, manifestBaseContentName, chunkID, language ) );
