	...
	, m_manager(NULL)
{
	...
	if( !m_manager )
	{
		RED_LOG_ERROR( FBXExportScene, TXT("Error: Unable to create FBX Manager!") );
		return;
	}

	String version = String::Printf(TXT("%ls"), ANSI_TO_UNICODE( m_manager->GetVersion() ) );
	// Print version
	RED_LOG_SPAM( FBXExportScene, TXT("Autodesk FBX SDK version %s"), ANSI_TO_UNICODE( m_manager->GetVersion() ) );
	...
	if ( !m_scene )
	{
		RED_LOG_ERROR( FBXExportScene, TXT("FBX: Unable to create FBX scene!") );
		m_manager->Destroy();
		return;
	}
...
Bool FBXExportScene::Save( const String& filePath, const EFBXFileVersion& version )
{
	...
	// Save scene
	{
		...
		{
			RED_LOG_ERROR( FBXExportScene, TXT("FBX: Call to FbxExporter::Initialize() failed.") );
			RED_LOG_ERROR( FBXExportScene, TXT("FBX: Error returned: %s"), ANSI_TO_UNICODE( lExporter->GetStatus().GetErrorString() ) );
			exportStatus = false;
		}
		else
		{
			...
			RED_LOG_SPAM( FBXExportScene, TXT("FBX file format version %d.%d.%d"), lMajor, lMinor, lRevision);

			// Export the scene.
			if ( !lExporter->Export( m_scene ) )
			{
				RED_LOG_ERROR( FBXExportScene, TXT("FBX: error exporting scene: %s"), ANSI_TO_UNICODE( lExporter->GetStatus().GetErrorString() ));
				exportStatus = false;
			}
			else
			{
	...
	if ( exportStatus )
	{
		...
		{
			RED_LOG_ERROR( FBXExportScene, TXT("Failed to save content manifest for file '%ls'"), xmlPath.GetPathString().AsChar() );
		}
	}
		...
		if ( localInstance )
		{
			...
			if ( const CMaterialInstance* matInstance = Cast< CMaterialInstance >( material ) )
			{
				...
				for ( const MaterialParameterInstance& param : parameters )
				{
					...
					{
						// hmmmm...
						RED_LOG_ERROR( FBXExportScene, TXT("Parameter %ls of type %ls is not supported!"), 
...
void FBXExportScene::ExportMesh( const Matrix& modelLocalToWorld, Bool exportSkinning, const CMesh* srcModel, const String& name  )
{
	...
	if (NULL != collision)
	{
		...
		RED_LOG_SPAM( FBXExportScene, TXT("%d collision shapes exported for '%s"), numShapes, name.AsChar() );
	}
...
Bool FBXExportScene::ExtractTextureData( const CBitmapTexture* bitmapTexture, String& outFileName )
{
	...
	if ( !exporter )
	{
		RED_LOG( FBX, TXT("No valid exporter for extension '%s' for %s")
	...
	RED_LOG( FBX, TXT("Exporting resource %s as %s.")
