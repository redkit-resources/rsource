	...
	String GetPackageID()
	{
		...
		if( productID.Empty() )
		{
			...
			for ( Uint32 i=0; i<tok.GetNumTokens(); ++i )
			{
				...
				if( token == TXT( "-xboxPackageID" ) )
				{
					productID = tok.GetToken( i + 1 );
					break;
				}
			}
		}
...
void CUserProfileManagerDurango::DisplayStore()  
{
	...
	if ( m_activeUserStub )
	{
		...
		Platform::String^ packageID = ref new Platform::String( Helper::GetPackageID().AsChar() );
		LOG_ENGINE(TXT("DisplayStore packageID=%ls"), packageID->Data());
		Product::ShowMarketplaceAsync( m_activeUserStub, ProductItemTypes::Game, packageID, ProductItemTypes::Durable );
	}
}
