...
#define ORBIS_TROPHY_SERVICE_LABEL_PARAM TXT( "-ps4TrophyServiceLabel" )

namespace Helper
{
	// Originally for DLC
	extern SceNpServiceLabel GetDLCServiceLabel();

	extern SceNpServiceLabel GetTrophyServiceLabel();
}
...
Bool CUserProfileManagerOrbis::Initialize()
{
	...
	if ( IsGOTY() )
	{
		ORBIS_SYS_CALL_RET( sceSysmoduleLoadModule( SCE_SYSMODULE_NP_COMMERCE ) );
	}
...
Bool CUserProfileManagerOrbis::Shutdown()
{
	...
	if ( IsGOTY() )
	{
		ORBIS_SYS_CALL_RET( sceSysmoduleUnloadModule( SCE_SYSMODULE_NP_COMMERCE ) );
	}
...
{
	SceNpServiceLabel serviceLabel = Helper::GetTrophyServiceLabel();
...
void CUserProfileManagerOrbis::DisplayStore()
{
	if ( !IsGOTY() )
	{
		LOG_ENGINE(TXT("DisplayStore NOT GOTY!"));
		return;
	}
	...
	if( !m_systemDialog )
	{
		...
		//#tbd: for language VO DLCs, special category label?
		// Show top-category for now
		param.serviceLabel = Helper::GetDLCServiceLabel(); // or 0?
		param.userId = m_userId;
